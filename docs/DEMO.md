...
- 제공이 종료된 무료 모델 설정을 교체했습니다. 기존 기본 설정값 `google/gemma-4-31b-it:free`는 유지하며, 현재 기본 요청 우선순위는 `nvidia/nemotron-3.5-lightning:free` → `google/gemma-4-31b-it:free` → `google/gemma-4-26b-a4b-it:free`입니다. 자동 유료 전환은 없습니다.
...
### 2026-09-22 AI 연결 개선의 확인 경계

- 기본 무료 모델 3개의 순서만 바꾸고 기존 명시적 모델 설정·서버 요청 1회·출력 4,000토큰을 보존합니다. 자동 재시도는 없습니다.
- 요청 접수부터 응답 헤더까지 새 20초 상한, 전체 서버/중복 예약 180초, 본문 수신 대기 15초, 클라이언트 전체 195초를 적용합니다. 최초 개행은 응답 시작용이며 실제 생성 내용이 없으면 실패합니다.
- 최종 응답의 인증 거절·제공자 요청 제한·제공자 시간초과를 구별합니다. 공개 HTTP 200이나 초기 개행은 AI 완료의 근거가 아닙니다. 실패한 의견은 부록에 넣지 않으며 기본 보고서는 계속 사용할 수 있습니다.
- 로컬 가상시계/합성 오류 검증은 운영 제공자 복구의 증거가 아닙니다. 09:04 운영 관측의 세 경로 실패(429→504→429)는 총괄 진단 기록에 남아 있으며, 새 코드의 운영 성공은 별도 배포·내용 검수로 판단합니다.

## 가상 사례 체험

메인 `예시로 바로 체험하기` 또는 `/review?example=area`에서5,000㎡부족을 확인하고 대지15,000㎡로 바꾸어 면적조건을 충족시킵니다. `/review?example=compare`는 가상후보A·B 두곳을 불러옵니다. 제약발견 → 조건변경 → 두후보비교 → 보고서 순서입니다. 사업조건·가격은 가상이며 실제필지/공급/인허가승인을 뜻하지 않습니다. 점수에 영향없는 규모·사업유형선택은 제거했습니다.
