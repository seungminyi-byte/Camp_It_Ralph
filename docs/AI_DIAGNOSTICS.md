...
입력 본문, 키, 헤더, 응답 원문, 오류 메시지·stack, 제공자 이름·고유 코드, 실제 응답 모델, 중복 요청 해시는 이 기록에 넣지 않습니다. HTTP 오류 body를 추가로 읽지 않습니다. 기존 파서 수락 조건과 기본 3개/명시 1개 무료 모델 경로, 서버 55초·본문 idle 15초·클라이언트 65초 제한은 유지합니다. 시간 로그의 상한은 실행 제한을 연장하는 설정이 아닙니다.
