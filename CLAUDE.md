...
- 런타임 외부 API 의존을 늘리지 않는다(지도 타일·LLM·**VWorld 점 조회 세 라우트**(`api/zoning.ts` 용도지역, `api/restrictions.ts` 규제구역, `api/disaster.ts` 재해위험지구)만 온라인 —
  규제구역은 배포본이 로그인·공공누리 4유형이라 번들이 불가해 2026-09-05 사용자 확정으로 예외; 재해위험지구 조회·15점 검토 감점은 2026-09-07 사용자 확정). 그 밖의 새 데이터는 빌드타임 JSON으로 번들.
...
- 2026-09-07 ARIA 통합: 사업 유형 10/40/100MW(기본40), 연안 근거 미확인 시 보류, 재해위험지구 15점 단일 감점·상한 없음·미확인 유지. 체크리스트 14행. 사전 생성 메모는 평가조건 서명 v3 일치 필요. 데이터 동기화만 할 때 `build_all.py --sync-only`.
