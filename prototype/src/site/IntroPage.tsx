...
const steps = [
  ...
  ['보고서로 정리', '첫 장 요약과 상세 근거를 보고서로 열고 A4 PDF로 저장해 후속 협의를 준비합니다.'],
];
...
function HomePage() {
  return <>
    <section className="home-hero">
      ...
      <figure className="product-example area-example" aria-label="가상부지 A 면적 계산 예시">
        <div className="example-heading"><span className="example-dot" />가상부지 A · 계산 체험 예시</div>
        <div className="area-example-body"><p className="area-example-kicker">대지면적을 바꾸면 결과는 어떻게 달라질까요?</p><h2>5,000㎡ 부족에서<br /><em>면적 충족으로.</em></h2><p>계획 연면적 30,000㎡ · 필요한 대지 15,000㎡</p>
          <div className="area-example-comparison"><div><span>변경 전 대지</span><strong>10,000㎡</strong><b>5,000㎡ 부족</b></div><span className="area-example-arrow" aria-hidden="true">→</span><div><span>변경 후 대지</span><strong>15,000㎡</strong><b>면적 충족</b></div></div>
          <a className="site-cta" href="/review?example=area">예시로 바로 체험하기 <span aria-hidden="true">↗</span></a><a className="text-link" href="/review?example=compare">두 후보 비교 체험하기 <span aria-hidden="true">→</span></a>
        </div><figcaption><strong>가상 조건을 이용한 계산 예시입니다.</strong><span>실제 지역의 점수나 사업 가능성을 평가한 결과가 아닙니다. <a href="/project#example">계산 조건 확인 →</a></span></figcaption>
      </figure>
    </section>
    <section className="site-section home-principles"><div className="section-intro"><span className="site-eyebrow">답을 내리기 전에, 근거부터</span><h2>확인된 것과<br />남은 질문을 함께.</h2><p>자료를 모으는 일에서 한 걸음 더.<br />어떤 후보를 추가 확인할지 논의할 수 있도록 돕습니다.</p></div><div className="principle-list"><article><span>01</span><div><h3>무엇이 제약인지</h3><p>공개자료에서 확인한 입지 제약과 주의할 사항를 출처와 함께 읽습니다.</p></div></article><article><span>02</span><div><h3>무엇을 아직 모르는지</h3><p>조회 실패와 자료 부족, 입력하지 않은 조건을 구분해 표시합니다. 확인하지 못한 내용을 ‘제약 없음’으로 표시하지 않습니다.</p></div></article><article><span>03</span><div><h3>다음에 무엇을 물을지</h3><p>공급기관·관할기관·설계 담당자에게 확인할 질문을 비교 결과와 보고서에 정리합니다.</p></div></article></div></section>
...
function ProjectPage() {
  return <>
    ...
    <section className="site-section editorial-grid"><div><span className="site-eyebrow">기획 배경</span><h2>주소 하나 뒤에<br />확인할 조건이 많습니다.</h2></div><div className="prose"><p>데이터센터 개발·사업검토 담당자는 후보지를 제안받으면 전력, 입지 제약, 재해, 주변 현황을 확인하고 면적·비용·공급 협의 조건을 보완해야 합니다.</p><p>자료는 여러 곳에 흩어져 있고 기준일과 공간 단위도 다릅니다. 자료를 찾은 뒤에도 확인한 사실과 아직 모르는 조건을 정리해 후보를 비교하고, 다음 담당자에게 확인할 질문을 넘기는 일이 남습니다.</p><p>이 도구는 그 첫 정리를 돕도록 설계했습니다. 실제 업무시간이나 절감 효과는 아직 측정하지 않았습니다.</p><p className="source-note">복수의 검토 조건이라는 배경은 <a href="https://www.iea.org/reports/key-questions-on-energy-and-ai/executive-summary" target="_blank" rel="noreferrer">IEA의 에너지·AI 분석</a>을 참고했습니다. GS건설의 업무 소요시간을 입증하는 자료는 아닙니다.</p></div></section>
    ...
    <section className="site-section"><div className="section-heading-copy"><span className="site-eyebrow">주요 기능</span><h2>근거와 입력 조건을 보고,<br />다음 검토를 준비합니다.</h2></div><div className="feature-grid"><article><h3>지도와 공개자료</h3><p>선택한 위치의 전력 공개 목록, 입지·재해 자료, 인구·가구·학교 등 주변 현황을 살펴봅니다. 지도 레이어와 실제 조회 상태를 구분합니다.</p></article><article><h3>면적·비용·협의 기록</h3><p>공통 사업조건과 부지별 조건을 나눠 입력합니다. 부족한 면적과 입력된 비용을 계산하고, 빠진 항목은 미확인으로 표시합니다.</p></article><article><h3>최대 4곳 비교</h3><p>담은 후보의 제약·미확인·면적·비용을 같은 형식으로 비교합니다. 비용 방식과 입력 범위가 다르면 차액 비교를 보류합니다.</p></article><article><h3>요약에서 상세 근거까지</h3><p>보고서 첫 장에서 위치, 제약, 중요한 미확인, 후보 비교와 다음 확인사항을 봅니다. 이어지는 상세에서 출처와 입력 가정을 확인합니다.</p></article></div></section>
    <section className="site-section" id="sources"><div className="section-heading-copy"><span className="site-eyebrow">자료와 활용 범위</span><h2>자료를 읽을 때,<br />기준일과 한계도 함께.</h2><p>각 검토 결과와 보고서에 자료별 출처·기준일·범위를 표시합니다.<br />온라인 조회시각은 원자료의 기준일을 뜻하지 않습니다.</p></div><div className="source-table-wrap" role="region" aria-label="공개자료 활용 범위"><table className="site-source-table"><caption className="sr-only">공개자료별 쓰임과 확인할 한계</caption><thead><tr><th scope="col">자료</th><th scope="col">확인할 수 있는 내용</th><th scope="col">추가로 확인할 사항</th></tr></thead><tbody>
      <tr><th scope="row"><a href="https://www.data.go.kr/data/15128065/fileData.do" target="_blank" rel="noreferrer">한전 공개 목록</a><span>행정구역별 등재</span></th><td data-label="확인할 수 있는 내용">공개 목록의 공급변전소 등재 여부</td><td data-label="추가로 확인할 사항">실제 연결 변전소·공급 가능량·계통 협의</td></tr>
      <tr><th scope="row"><a href="https://www.vworld.kr/" target="_blank" rel="noreferrer">VWorld</a><span>온라인 조회 상태·시각 별도 표시</span></th><td data-label="확인할 수 있는 내용">용도지역·규제구역·재해위험지구 조회 결과</td><td data-label="추가로 확인할 사항">최신 고시 도면·개별 법적 적용·조회 누락</td></tr>
      <tr><th scope="row"><a href="https://sgis.kostat.go.kr/" target="_blank" rel="noreferrer">SGIS 인구·가구</a><span>2024년 · 1km 격자</span></th><td data-label="확인할 수 있는 내용">반경 내 격자 중심점의 참고 합계</td><td data-label="추가로 확인할 사항">결측·부분 합계·실제 거주와 수용성</td></tr>
      <tr><th scope="row"><a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer">OpenStreetMap</a><span>위치 참고자료</span></th><td data-label="확인할 수 있는 내용">지도·변전소 등 위치와 직선거리</td><td data-label="추가로 확인할 사항">실제 인입 경로·시설 속성·공급 조건</td></tr>
      <tr><th scope="row">보호지역·지형 번들<span>국립공원 2024 / KDPA 2016 / SRTM 2000 촬영</span></th><td data-label="확인할 수 있는 내용">보호지역 중첩과 주변 지형 참고</td><td data-label="추가로 확인할 사항">이후 변경·매립·고시 경계·현장 지반</td></tr>
      <tr><th scope="row">사용자 입력<span>설계·비용·협의 가정</span></th><td data-label="확인할 수 있는 내용">선택한 가정의 면적·비용·금융비용</td><td data-label="추가로 확인할 사항">실설계·견적·공급기관 확인과 확약</td></tr>
    ...
    <section className="site-section editorial-grid" id="example"><div><span className="site-eyebrow">가상부지 A · 계산 체험 예시</span><h2>면적 조건을 바꾸고,<br />계산 결과를 확인하세요.</h2></div><div className="prose"><p>계획 연면적 30,000㎡, 용적률 200%, 건폐율 50%, 지상 4층을 가정합니다. 용적률로 계산한 필요 대지와 층수·건폐율로 계산한 필요 대지가 모두 15,000㎡입니다.</p><p>대지 10,000㎡에서는 5,000㎡가 부족하고, 대지를 15,000㎡로 바꾸면 이 면적 조건을 충족합니다. 면적 충족은 전력 공급·인허가·사업 적합성을 보장하지 않습니다. 특정 지역의 실제 평가가 아닌 가상 계산 예시입니다.</p><a className="text-link" href="/review?example=area">예시로 바로 체험하기 →</a><br /><a className="text-link" href="/review?example=compare">두 후보 비교 체험하기 →</a></div></section>
