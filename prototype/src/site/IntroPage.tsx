...
function HomePage() {
  return <>
    <section className="home-hero">
      ...
      <figure className="product-example"><div className="example-heading"><span className="example-dot" />실제 제품의 검토 결과<span>합성 검증 예시</span></div><a href="/images/product/review-example.webp" target="_blank" rel="noreferrer" aria-label="합성 검증 결과 화면 크게 보기 (새 탭)"><picture><source media="(max-width: 1023px)" srcSet="/images/product/review-example-mobile.webp" width="716" height="1042" /><img src="/images/product/review-example.webp" width="1130" height="978" fetchPriority="high" alt="실제 제품 결과 예시. 세종특별자치시 반곡동의 합성 입력에서 면적 부족과 공개자료·전력·용수 미확인을 함께 표시합니다." /></picture></a><figcaption><strong>공개자료·합성 검증 예시 · 실제 부지 판정 아님</strong><span>고정 조회 응답과 합성 사업조건으로 생성한 결과입니다. <a href="/project#example">예시 조건 확인 →</a></span></figcaption></figure>
    </section>
...
function ProjectPage() {
  return <>
    ...
    <section className="site-section editorial-grid" id="example"><div><span className="site-eyebrow">메인 화면 예시의 조건</span><h2>실제 제품 결과,<br />합성 검증 입력.</h2></div><div className="prose"><p>메인 예시는 세종 지역의 공개 좌표(36.4967, 127.3007)에 계획 연면적 30,000㎡, 확보 대지 10,000㎡, 용적률 200%, 건폐율 50%, 지상 4층을 입력한 제품 화면 발췌입니다.</p><p>저장소의 공개자료와 고정된 온라인 조회 응답, 합성 사업조건을 사용했습니다. 2026년 9월 21일 생성한 검증 예시이며 해당 위치의 실제 용도지역·규제 상태나 사업 가능성에 관한 판단이 아닙니다.</p><a className="text-link" href="/images/product/review-example.webp" target="_blank" rel="noreferrer">예시 화면 크게 보기 ↗</a></div></section>
