...
import { BUSINESS_TYPE_LABELS, PROJECT_SCALE_LABELS } from '../lib/reviewInputs';
...
function OnlineStatus({ input, at }: { input: Pick<ScoreInput, 'zoning' | 'restrictions' | 'disaster'>; at?: string }) {
  ...
  return <ul className="report-online-status">{values.map(([label, value]) => <li key={label}>
    <b>{label}</b>: {!value ? '조회 미확인' : value.stale || (value.fetchedAt && !isEvidenceFresh({ ...value, complete: true }, at ? Date.parse(at) : Number.POSITIVE_INFINITY)) ? '이전 관찰 보존 · 재확인 필요' : value.complete ? '조회 완료' : '일부 조회 미완료'}
    {value?.fetchedAt && ` · API 조회 ${reportTime(value.fetchedAt)}`}
    {value?.previousFetchedAt && ` · 이전 관찰 ${reportTime(value.previousFetchedAt)}`}
    {!!value?.failed?.length && ` · 미완료 레이어 ${value.failed.join(', ')}`}
  </li>)}</ul>;
}
function RestrictionDetails({ result }: { result: ScoreResult }) {
  return result.restriction.hits.length > 0 && <section className="report-restrictions"><h3>확인한 보호·규제구역 관찰 전문</h3>
...
function IssueDetails({ result }: { result: ScoreResult }) {
  return <><h3>주요 제약과 미확인 사항 전문</h3><ul>{result.review.issues.map((issue, i) => <li key={i}><b>{issue.title}</b>: {issue.detail}</li>)}</ul><h3>전체 다음 확인사항</h3><ol>{result.review.actions.map((action, i) => <li key={i}>{action}</li>)}</ol></>;
}
export function ChecklistReport({ data, input, result: r, rows, site, landUseSource, zoningName, entries = [], currentPinId = null, snapshotId = '미리보기', capturedAt, memo: candidateMemo, memoEligible = false, includeAiAppendix = false, generatedBy, generatedAt, variant }: ReportProps) {
  ...
  const leadIssues = (issues: typeof groups.constraints, empty: string) => issues.length ? <><ul>{issues.slice(0, 3).map((issue, i) => <li key={i}>{excerpt(issue.title, 48)}</li>)}</ul>{issues.length > 3 && <p>외 {issues.length - 3}건 · 상세 전문 참조</p>}</> : <p>{empty}</p>;
  ...
  return <article className={`business-report report-${variant}`} data-snapshot-id={snapshotId}>
    <section className="report-first-page">
      ...
      <section className="report-brief-inputs"><h2>입력 조건·계산 요약</h2><p>{PROJECT_SCALE_LABELS[p.type]} · {BUSINESS_TYPE_LABELS[p.businessType]} · 수전 {p.targetMw === null ? '미입력' : `${p.targetMw}MW`} · {p.development === 'new' ? '신축' : '기존 건물 전환'}</p><p>필요 연면적 {fmtArea(r.area.requiredAreaM2)} · {p.development === 'new' ? `최소 대지 ${fmtArea(r.area.minimumLandM2)} · 확보 ${fmtArea(r.conditions.landAreaM2)}` : `확보 건물 ${fmtArea(r.conditions.existingAreaM2)}`}{r.area.status === 'shortfall' && ` · 부족 ${fmtArea(r.area.shortfallM2)}`}</p><p>{r.area.status === 'unknown' && '면적 계산 보류 · '}{r.businessCost.label} {fmtKrw(r.businessCost.amountKrw)}{!r.businessCost.complete && ' · 비용 범위 미완료'}</p><p>평균 차입잔액 {fmtKrw(r.finance.debtKrw)} · 금융비용 {financeCell ? `${Number.isFinite(financeCell.annualRate) ? Number((financeCell.annualRate * 100).toFixed(4)) + '%' : '금리 미확인'} / ${financeCell.months}개월: ${fmtKrw(financeCell.costKrw)}` : '계산 보류'} <span>(가운데 가정, 전체 표는 상세)</span></p></section>
      <section className="report-comparison"><h2>담은 후보 비교 요약 · {entries.length}곳</h2><p>{currentComparisonMessage(entries, currentPinId)}{entries.length === 1 && ' · 2곳 이상에서 후보 간 비교 가능'}</p>
        ...
        {!!entries.length && <p>공통 사업·설계·금리 가정. 각 후보의 조건·근거·전문은 뒤쪽 상세.</p>}
      </section>
      <section className="report-next"><h2>다음 확인사항</h2><ol>{r.review.actions.slice(0, 3).map((action, i) => <li key={i}>{excerpt(action, 112)}</li>)}</ol>{r.review.actions.length > 3 && <p>외 {r.review.actions.length - 3}건 · 엔진 제시 순서, 상세에서 전체 확인</p>}</section>
      ...
      <footer>스크리닝 참고용, 한전 공식 검토·법률 판단 대체 불가. 요약 1쪽 · 다음 쪽부터 상세 전문</footer>
    </section>
