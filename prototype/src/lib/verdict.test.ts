...
function result(grade = 'B') {
  return { site: { eligible: true, status: 'ok', detail: '육지' }, terrain: null,
    ...
    disaster: { status: 'none', hits: [], deduction: 0 },
    restriction: { level: 'none', hits: [], checked: { bundled: true, vworld: 'ok' } },
...
describe('screening verdict safeguards', () => {
  ...
  it('keeps failed or partial regulatory lookups unconfirmed on every summary surface', () => {
    for (const vworld of ['partial', 'none'] as const) {
      const r = result();
      r.restriction.checked.vworld = vworld;
      expect(siteVerdict(r).label).toBe('조건부 검토');
    }
    const r = result();
