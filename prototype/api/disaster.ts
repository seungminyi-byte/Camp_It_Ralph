...
// Point lookup of the VWorld disaster-risk-zone polygon layer. This endpoint is deliberately
// diagnostic for now: it reports whether the selected coordinate intersects LT_C_UP201, but does
// not feed the scoring engine until the live response attributes have been verified.
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    return textResponse(
      `vworld disaster-risk lookup failed: ${error instanceof Error ? error.message : String(error)}`,
      502,
    );
  }
}
